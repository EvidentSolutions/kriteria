...
internal inline fun transactionalTest(db: DatabaseContext, block: context(Tx) () -> Unit) {
...
internal inline fun testWithDatabaseContext(db: DatabaseContext, block: context(DatabaseContext) () -> Unit) {
    context(db) {
        block()
    }
}
...
context(tx: Tx)
internal fun truncateTables() {
    truncateTables(generatedEntityMetas)
}

context(tx: Tx)
internal fun truncateTable(meta: EntityMeta<*, *>) {
    truncateTables(listOf(meta))
}

context(tx: Tx)
internal fun truncateTables(metas: List<EntityMeta<*, *>>) {
...
internal fun buildSessionFactory(): SessionFactory {
